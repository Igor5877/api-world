...
class CRUDMainIslandQueue:
    ...
    async def add_to_queue(self, db_session: AsyncSession, *, player_uuid: str, player_name: Optional[str] = None) -> IslandQueue:
        ...
        if existing_entry:
            if existing_entry.status == QueueItemStatusEnum.PENDING:
                logger.info(f"Player {player_uuid} already in queue with PENDING status.")
                return existing_entry
            else:  # e.g., was PROCESSING and failed, or some other state, re-queue as PENDING
                stmt = (
                    update(IslandQueue)
                    .where(IslandQueue.player_uuid == player_uuid)
                ...
                logger.info(f"Re-queued player {player_uuid} with status PENDING.")
    ...
    async def remove_from_queue(self, db_session: AsyncSession, *, player_uuid: str) -> bool:
        ...
        if deleted_count > 0:
            logger.info(f"Removed player {player_uuid} from island_queue.")
        else:
            logger.info(f"Player {player_uuid} not found in island_queue for removal or already removed.")
    ...
    async def update_queue_item_status(self, db_session: AsyncSession, *, player_uuid: str, status: QueueItemStatusEnum) -> Optional[IslandQueue]:
        ...
        if updated_item:
            logger.info(f"Updated island_queue status for player {player_uuid} to {status.value}.")
            return updated_item
        else:
            logger.warning(f"Could not find player {player_uuid} in island_queue to update status to {status.value}.")
            return None
