...
public class MarketSyncTask implements Runnable {
    ...
    public void run() {
        ...
        // Виправлено: for-each замість forEach лямбди, щоб уникнути
        // "incompatible types" через boxing Long/long в BiConsumer
        for (var entry : availableStacks) {
            ...
            if (key instanceof AEItemKey itemKey) {
                ...
                MarketData.Entry marketEntry = marketData.stocks.get(item);
                if (marketEntry != null) {
                    JsonObject obj = new JsonObject();
                    obj.addProperty("item_id", ForgeRegistries.ITEMS.getKey(item).toString());
                    obj.addProperty("amount", amount);
                    obj.addProperty("price", marketEntry.base);
                    if (itemKey.hasTag()) {
