...
## 6. Бази даних та синхронізація мільярдів предметів (Економіка / AE2)
*   **Сильна сторона системи:** Використання `BigInteger` для поля `quantity` та `ON DUPLICATE KEY UPDATE` (UPSERT) дозволяє ефективно працювати з величезними стаками предметів. Також відмінним архітектурним рішенням є механізм хешування інвентарю перед синхронізацією: якщо предмети не змінилися, база даних взагалі не чіпається.
*   **Слабке місце (Масштабування унікальних предметів):** У методі `sync_island_inventory` (файл `crud_market.py`) використовується конструкція `DELETE ... WHERE NOT IN (величезний список туплів)`. Якщо гравець покладе в AE2 систему тисячі або десятки тисяч **унікальних** предметів (наприклад, броня з різними NBT тегами або поломкою), цей список `visible_pairs` стане гігантським. Це призведе до переповнення пам'яті сервера бази даних або перевищення ліміту розміру SQL-запиту (max_allowed_packet).
*   **Як має бути (Рішення проблеми):** Замість `DELETE ... WHERE NOT IN (...)`, варто запровадити поле `last_sync_time` в таблицю `market_items`.
    1. Під час операції `UPSERT` для актуальних предметів оновлювати `last_sync_time = NOW()`.
    2. Замість гігантського `NOT IN`, виконувати просту операцію очищення: `DELETE FROM market_items WHERE team_id = X AND last_sync_time < NOW() - INTERVAL 1 MINUTE`. Це працює за $O(1)$ розміру пакета і не залежить від кількості типів предметів.
