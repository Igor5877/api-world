...
## Нове 2026-07-06 (частина Б): оновлення ядра сервера + чистка логів

Тепер через кампанії можна оновлювати і **ядро** (Forge/Arclight + ServerWrapper):

- **Core-файли** (`run.sh`, `start.sh`, `user_jvm_args.txt`, `ServerWrapper.jar`,
  `ServerWrapperInline.jar`, `ServerWrapperConfig.json`) — пушаться при **кожному
  hard-оновленні**, якщо лежать у репо оновлень. Дрібні, тож завжди сходяться.
- **`libraries/`** — пушиться з повним очищенням **тільки коли diff кампанії
  її торкнувся** (сотні МБ, ганяти щоразу нема сенсу).
- Будь-яка зміна ядра ⇒ **примусовий restart** і **примусовий LXD-снапшот**
  (навіть при `UPDATE_SNAPSHOT_MODE=never`) — бо відкат libraries можливий
  лише через `lxc restore` зі снапшота, файлово вона не бекапиться.
  Дрібні core-файли бекапляться/відкочуються як звичайні файли.
- `world/`, `server.properties`, `eula.txt` тощо — як і раніше, не чіпаються.

**Як оновити ядро (напр. Forge 47.4.0 → 47.5.x):**
1. У локальній копії встанови нову версію (installer) в окрему директорію.
2. Скопіюй у репо оновлень: `libraries/` (цілком), новий `run.sh` (Forge пише
   туди шлях до нової версії в `@libraries/.../unix_args.txt`), за потреби
   `user_jvm_args.txt` і wrapper-файли.
3. Коміть, тегни (`vX.Y.Z` — рестарт і так буде примусовим), пушни тег.
4. Кампанія сама зупинить острови, зробить снапшоти, заллє ядро.

**Чистка логів:** під час кожного hard-оновлення (і для спавна/шаблона) в
контейнері чистяться `logs/` і `crash-reports/` — лишаються останні 20 файлів
(+ `latest.log`/`debug.log`). Постійна гігієна диска без ручної роботи.

⚠️ Разово закоміть у репо оновлень поточні core-файли (щоб перший же hard-тег
не «оновив» острови старішими файлами, ніж у них є): бери їх з еталона
(Nextcloud/шаблонний контейнер), НЕ з довільного острова.
