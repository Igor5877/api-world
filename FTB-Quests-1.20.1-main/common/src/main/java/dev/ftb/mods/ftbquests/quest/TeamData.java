...
public class TeamData {
	public static final TeamData UNLOADED = new TeamData(new UUID(0L, 0L), null, "UNLOADED") {
		@Override
	...
	public static IslandData get(Player player) {
		return FTBQuestsAPI.api().getQuestFile(player.getCommandSenderWorld().isClientSide()).getOrCreateIslandData(player);
	}
	...
	public void saveIfChanged() {
		if (shouldSave && file instanceof ServerQuestFile sqf) {
			...
			SNBT.write(path.resolve(islandId + ".snbt"), serializeNBT());
			shouldSave = false;
		}
	}
	...
	public String toString() {
		return name.isEmpty() ? islandId.toString() : name;
	}
			...
			if (time == null) {
				if (started.remove(questId) >= 0L) {
					...
					if (file.isServerSide()) {
						new ObjectStartedResetMessage(islandId, questId).sendTo(getOnlineMembers());
					}
				...
				if (started.put(questId, time.getTime()) == 0L) {
					...
					if (file.isServerSide()) {
						new ObjectStartedMessage(islandId, questId).sendTo(getOnlineMembers());
					}
	...
	public boolean setCompleted(long id, @Nullable Date time) {
		...
		if (time == null) {
			if (completed.remove(id) >= 0L) {
				...
				if (file.isServerSide()) {
					new ObjectCompletedResetMessage(islandId, id).sendTo(getOnlineMembers());
				}
			...
			if (completed.put(id, time.getTime()) == 0L) {
				...
				if (file.isServerSide()) {
					new ObjectCompletedMessage(islandId, id).sendTo(getOnlineMembers());
				}
	...
	public boolean setRewardsBlocked(boolean rewardsBlocked) {
		if (rewardsBlocked != this.rewardsBlocked) {
			...
			markDirty();
			if (file.isServerSide()) {
				new SyncRewardBlockingMessage(islandId, rewardsBlocked).sendTo(getOnlineMembers());
			}
			return true;
		}
		return false;
	...
	public boolean hasUnclaimedRewards(UUID player, QuestObject object) {
		...
		if (b == BOOL_UNKNOWN) {
			b = object.hasUnclaimedRewardsRaw((IslandData) this, player) ? BOOL_TRUE : BOOL_FALSE;
	...
	public boolean claimReward(UUID player, Reward reward, long date) {
		...
		if (!claimedRewards.containsKey(key)) {
			...
			if (file.isServerSide()) {
				new ClaimRewardResponseMessage(islandId, player, reward.id).sendTo(getOnlineMembers());
			}

			reward.getQuest().checkRepeatable((IslandData) this, player);
	...
	public boolean resetReward(UUID player, Reward reward) {
		if (!locked && claimedRewards.removeLong(QuestKey.forReward(player, reward)) != 0L) {
			...
			if (file.isServerSide()) {
				new ResetRewardMessage(islandId, player, reward.id).sendTo(getOnlineMembers());
			}
	...
	public SNBTCompoundTag serializeNBT() {
		...
		nbt.putString("uuid", UUIDTypeAdapter.fromUUID(islandId));
	...
	public void deserializeNBT(SNBTCompoundTag nbt) {
		...
		for (String key : ppdTag.getAllKeys()) {
			try {
				...
				FTBQuests.LOGGER.error("ignoring invalid player ID {} while loading per-player data for team {}", key, islandId);
			}
		}
	...
	public int getRelativeProgress(QuestObject object) {
		...
		return object.getRelativeProgressFromChildren((IslandData) this);
	}
	...
	private boolean checkDepsCached(Quest quest, Long2ByteMap cache, ToBooleanBiFunction<Quest,IslandData> checker) {
		...
		if (res == BOOL_UNKNOWN) {
			res = checker.applyAsBoolean(quest, (IslandData) this) ? BOOL_TRUE : BOOL_FALSE;
	...
	public final void setProgress(Task task, long progress) {
		...
		if (prevProgress != progress || progress == 0L && isStarted(task)) {
			...
			if (file.isServerSide()) {
				...
				new UpdateTaskProgressMessage((IslandData) this, task.id, progress).sendTo(onlineMembers);

				if (prevProgress == 0L) {
					task.onStarted(new QuestProgressEventData<>(now, (IslandData) this, task, onlineMembers, Collections.emptyList()));
				}
	...
	public void markTaskCompleted(Task task) {
		...
		task.onCompleted(new QuestProgressEventData<>(new Date(), (IslandData) this, task, onlineMembers, notifiedPlayers));
	...
	public boolean setLocked(boolean newLocked) {
		if (locked != newLocked) {
			...
			if (file.isServerSide()) {
				new SyncLockMessage(islandId, locked).sendTo(getOnlineMembers());
			}
	...
	private Optional<PerPlayerData> getOrCreatePlayerData(Player player) {
		...
		if (!perPlayerData.containsKey(player.getUUID()) && file.isPlayerOnTeam(player, (IslandData) this)) {
		...
		return getOrCreatePlayerData(player).map(playerData -> {
			if (playerData.canEdit != newCanEdit) {
				...
				if (file.isServerSide() && player instanceof ServerPlayer sp) {
					new SyncEditingModeMessage(islandId, newCanEdit).sendTo(sp);
				}
				return true;
			}
			return false;
	...
	public boolean isExcludedByOtherQuestline(QuestObject qo) {
		if (qo instanceof Excludable e) {
			...
			boolean excluded = e.isQuestObjectExcluded((IslandData) this);
